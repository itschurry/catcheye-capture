...
- 저장 완료 GPIO 펄스 출력
- Heartbeat LED GPIO 13번 1초 간격 점멸
- catcheye-studio viewer용 WebSocket 프레임 송출
- HTTP API 기반 수동 캡처, 저장 이미지 조회, 녹화, RGB 카메라 속성 조회/변경
...
CATCHEYE_CAPTURE_HEARTBEAT_LED_GPIO=13 \
...
| `--heartbeat-led-gpio <line>` | 앱 Heartbeat LED 출력 GPIO line이다. 기본값은 `13`이고 `-1`이면 비활성화한다. |
| `--heartbeat-led-active-low` | Heartbeat LED 출력을 active-low로 구동한다. |
| `--heartbeat-led-interval-ms <ms>` | Heartbeat LED 점멸 간격이다. 기본값은 `1000`이다. |
...
성공한 변경은 `--camera-properties` JSON 파일에 바로 저장된다.
